...
if task_batch_file.exists():
    task_batch_file.unlink()
...
for task in results.tasks:
    ...
    if id in ids:
        print(f"skipping duplicate task {task}")
        continue
    ids.append(id)
...
with open(task_batch_file, 'wb') as file:
    for line in good_lines:
        file.write(line + b"\n")

print(len(good_lines), "valid JSON lines written to", task_batch_file)
...
outdir = outdir_fulltext.parent / 'task_results'
